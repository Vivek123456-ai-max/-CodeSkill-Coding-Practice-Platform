...
# Process Book 1: Pattern Printing Mastery (#001 to #100)
for idx, p in enumerate(pattern_problems, 1):
    num_str = f"{idx:03d}"
    uuid_str = f"00000000-0000-4000-8000-{idx:012d}"
    slug = f"problem-{num_str}-pattern-{p['title'].lower().replace(' ', '-').replace('/', '-').replace('&', 'and')}"
...
for idx, p in enumerate(loop_problems, 101):
    num_str = f"{idx:03d}"
    uuid_str = f"00000000-0000-4000-8000-{idx:012d}"
    slug = f"problem-{num_str}-loop-{p['title'].lower().replace(' ', '-').replace('/', '-').replace('&', 'and')}"
...
# Sort strictly by order_index 1..200
all_problems.sort(key=lambda x: x['order_index'])
...
with open('supabase/insert_all_problems.sql', 'w') as f:
    f.write("-- ==============================================================================\n")
    f.write("-- 200 COMPLETE PRACTICE PROBLEMS IN PERFECT NUMERIC ORDER (#001 TO #200)\n")
    f.write("-- ==============================================================================\n\n")
    f.write("insert into public.problems (id, order_index, title, slug, description, difficulty, category, tags, source, sample_test_cases, starter_templates)\nvalues\n")
    f.write(",\n".join(sql_inserts))
    f.write("\non conflict (id) do update set\n")
    f.write("  order_index = excluded.order_index,\n  title = excluded.title,\n  description = excluded.description,\n  difficulty = excluded.difficulty,\n  category = excluded.category,\n  tags = excluded.tags,\n  sample_test_cases = excluded.sample_test_cases,\n  starter_templates = excluded.starter_templates;\n")

print(f"Generated {len(all_problems)} serial problems (#001 to #200)")
